...
func TestCompleteMilestoneWithAudit(t *testing.T) {
	db, err := storage.Open(context.Background(), "file:TestCompleteMilestoneWithAudit?mode=memory&cache=shared")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { db.Close() })
	if _, err := db.Exec(`INSERT INTO users(id,email,password_hash,role,created_at) VALUES(1,'lead-TestCompleteMilestoneWithAudit@example.test','x','platform_lead',CURRENT_TIMESTAMP)`); err != nil {
		t.Fatal(err)
	}
	s := NewService(db)
	p, err := s.Create(context.Background(), 1, "CompleteMilestoneWithAudit平台", "忻州", "先进技术", time.Now().Year()+1, 1000, "TestCompleteMilestoneWithAudit-idempotent")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = s.CompleteMilestoneWithAudit(context.Background(), 1, p.ID, 2)
	if err == nil {
